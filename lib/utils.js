...
export function formatRxCode(id, dateInput) {
  if (!id) return 'AYUSH-RX-2026-0001';
  let year = 2026;
  if (dateInput) {
  ...
  const cleanId = String(id).replace(/[^a-zA-Z0-9]/g, '').slice(0, 6).toUpperCase() || '001';
  return `AYUSH-RX-${year}-${cleanId}`;
}
